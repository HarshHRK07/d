...
def modify_request(flow: http.HTTPFlow):
    ...
    try:
        ...
        if data_removed:
            ctx.log.info("Sensitive data removed from request.")
        else:
            ctx.log.info("No sensitive data found in request.")
            
    except Exception as e:
        ctx.log.error(f"Error processing request: {e}")

def serve_index(flow: http.HTTPFlow):
    """
    Serves the index.html file when the specific URL is requested.
    """
    if flow.request.pretty_url == "https://HRK":
        try:
            ...
            ctx.log.error(f"Error serving index file: {e}")
            flow.response = http.HTTPResponse.make(
                500,
...
def request(flow: http.HTTPFlow):
    """
    This function intercepts and modifies requests to remove sensitive data, and serves an index file for a specific URL.
    """
    if flow.request.pretty_url == "https://HRK":
        serve_index(flow)
    elif flow.request.method in ["POST", "GET", "OPTIONS"]:  # Include GET and OPTIONS requests
        modify_request(flow)
